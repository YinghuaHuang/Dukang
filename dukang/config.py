...
from dukang.models import catalog
...
def load_model_from_file(path, model_config=None):
  ...
  if model_config is None:
    ...
    model = module.model(model_config=model_config)
  del sys.path_importer_cache[os.path.dirname(module.__file__)]
  del sys.modules[module.__name__]
  return model

def load_model_from_catalog(name, model_config=None):
...
def load_model(model_dir,
  ...
  if model_name_or_path:
    ...
    if model_file:
      model = load_model_from_file(model_file, model_config=model_config)
      if serialize_model:
        tf.io.gfile.copy(model_file, model_description_path, overwrite=True)
    elif model_name:
      model = load_model_from_catalog(model_name, model_config=model_config)
      if serialize_model:
    ...
    model = load_model_from_file(model_description_path, model_config=model_config)
  else:
